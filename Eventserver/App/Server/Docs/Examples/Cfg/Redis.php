...
class Redis{
    ...
    public static $default = array('host'=>'192.168.25.9',
                                   'port'=>6379,
                                   'timeout'=>1,
                                   ...
                                   'options'=>array(\Redis::OPT_SERIALIZER => 'msgpack',
                                                    \Redis::OPT_READ_TIMEOUT => 0.01,
                                                   )
                                  );

    /**
     * This redis instance should not use permament storage. It's for caches only.
     *
     * @var array
     */
    public static $cache = array('host'=>'192.168.25.9',
            'port'=>6379,
            'timeout'=>1,
            'db_index'=>5,//integer number. null means default db
            'password'=>null,//leave it NULL or remove this item if no authentication is required.
            'options'=>array(\Redis::OPT_SERIALIZER => 'msgpack',
                             \Redis::OPT_READ_TIMEOUT => 0.01,
            )
